...
contains
    ...
    subroutine calculate_hamiltonian(time_num, time_step, freq_rotor, gtensor, &
        ...
        real(kind=8), intent(out) :: energies(time_num, 4), density_mat(4, 4)
        real(kind=8), intent(out) :: eig_vector(time_num, 4,4), eig_vector_inv(time_num, 4,4)
        ...
        real(kind=8) :: eigval(time_num, 4), dummy(1,1), work(16), work_inv(8)
        ...
        real(kind=8) :: temp(time_num, 4,4)
        real(kind=8), dimension(4, 4) :: test1, test2
        ...
        do count = 1, time_num
            ...
            call DGEEV('N', 'V', 4, test1, 4, eigval(count, :), dummy, dummy, 4, eig_vector(count, :, :), &
                    4, work, 16, info)
        end do
        energies = eigval
!        write(6, *) eig_vector(1, :, :)
        ...
        do count = 1, size(eig_vector, 1)
            ...
            call DGETRF(4, 4, test2, 4, ipiv, info_inv)
            call DGETRI(4, test2, 4, ipiv, work_inv, 8, info_inv)
...
!        write(6, *) eig_vector_inv(1, 1, 1)
    ...
    subroutine liouville_propagator(time_num, time_step, electron_frequency, freq_nuclear_1, microwave_amplitude, &
        ...
        spin2_i_y = kron_complex(identity_size4_complex, spin_y)
...
!            end do
            ...
            propagator(count, :, :) = matmul(eigvectors_inv_complex, matmul(mat_exp, eigvectors_complex))

            write(6, *) propagator(count, :, :)
