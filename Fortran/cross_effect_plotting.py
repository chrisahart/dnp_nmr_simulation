...
def plot_all(directory):
    ...
    pol_elec1 = np.loadtxt('{}{}'.format(directory, '/pol_s1_z.out'))
    pol_elec2 = np.loadtxt('{}{}'.format(directory, '/pol_s2_z.out'))
    pol_elec = pol_elec1 + pol_elec2
    pol_iz_final = np.loadtxt("out/pol_iz_final.out")
    pol_nuc_rotor = np.loadtxt("out/pol_i_z_rot.out")
    pol_elec_rotor1 = np.loadtxt("out/pol_s1_z_rot.out")
    pol_elec_rotor2 = np.loadtxt("out/pol_s2_z_rot.out")
    ...
    energy_rotor_5 = np.loadtxt("out/energies_5.out") / 1E6
    energy_rotor_6 = np.loadtxt("out/energies_6.out") / 1E6
    energy_rotor_7 = np.loadtxt("out/energies_7.out") / 1E6
    energy_rotor_8 = np.loadtxt("out/energies_8.out") / 1E6
    ...
    # Plot sub rotor dynamics if data files exist
        ...
        # Plot energy
        subfig_x3.plot(angle, energy_rotor_1, 'k')
        subfig_x3.plot(angle, energy_rotor_2, 'r')
        subfig_x3.plot(angle, energy_rotor_3, 'b')
        subfig_x3.plot(angle, energy_rotor_4, 'g')
        subfig_x3.plot(angle, energy_rotor_5, 'orange')
        subfig_x3.plot(angle, energy_rotor_6, 'm')
        subfig_x3.plot(angle, energy_rotor_7, 'brown')
        subfig_x3.plot(angle, energy_rotor_8, 'grey')
